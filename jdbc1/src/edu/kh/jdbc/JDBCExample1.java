...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		/* EMPLOYEE 테이블에서 사번, 이름, 부서코드, 직급코드, 급여, 입사일 조회해서 
		 * Eclipse 콘솔에 출력 */
		...
		try {
			...
			String sql = "SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, SALARY, HIRE_DATE "
					+ "FROM EMPLOYEE";
			// JDBC에서 SQL 작성 시 ;를 넣으면 안됨 !!!
			// 문장을 이어붙이는 구문이기 때문에 문장 끝나고 뒤에 띄어쓰기 반드시 삽입 !!
			// "SQL 명령어가 올바르게 종료되지 않았다는 오류 발생"
			...
			/* 6. 조회 결과가 담겨있는 ResultSet을 커서를 이용해 1행씩 접근하며
			 * 각 행의 컬럼 값 얻어오기 */
			
			while(rs.next()) {
				// rs.next() : 커서를 한 행씩 이동시켜 해당 행이 있으면 true 아니면 false
				
				// rs.get자료형(컬럼명 | 순서) -> 현재 접근한 행의 컬럼 값 얻어오기
				
				// [java]             [db]
				// String         CHAR, VARCHAT2
				// int, long      NUMBER(정수만)
			...
			// 추가한 라이브러리가 없거나, 클래스 경로를 잘못 작성한 경우
			e.printStackTrace();
		} finally {
